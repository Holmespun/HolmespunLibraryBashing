...
DatStamp	?= $(shell date '+%Y%m%d_%H%M%S')

LibraryDSpec	?= /opt/holmespun/HolmespunLibraryBashing/
ExecuteDSpec	?= /usr/bin
...
install :
	@echo "make $@"
	$(QUIET) InstallLibraryDSpec=$(LibraryDSpec) InstallExecuteDSpec=$(ExecuteDSpec) Support/install_from_make.bash
